// Type definitions for pwDB 2.x
// Project: https://github.com/Pond-Water/pwDB-types
// Definitions by: Chippewa Software Technology <https://www.chipsofttech.com>
...
declare class Pwdb {
     ...
     */
    ensureIndex(options: Pwdb.EnsureIndexOptions, cb?: (err: Error) => void): void;
    ...
    count(query: any): Pwdb.CursorCount;
    ...
    find<T>(query: any, projection?: T): Pwdb.Cursor<T>;
     ...
     */
    update(query: any, updateQuery: any, options?: Pwdb.UpdateOptions, cb?: (err: Error, numberOfUpdated: number, upsert: boolean) => void): void;
     ...
     * @api private Use Datastore.update which has the same signature
     */
    update<T>(query: any, updateQuery: any, options?: Pwdb.UpdateOptions, cb?: (err: Error, numberOfUpdated: number, affectedDocuments: any, upsert: boolean) => void): void;
     ...
     */
    remove(query: any, options: Pwdb.RemoveOptions, cb?: (err: Error, n: number) => void): void;
    remove(query: any, cb?: (err: Error, n: number) => void): void;
}

declare namespace Pwdb {
    interface Cursor<T> {
    ...
    interface DataStoreOptions {
        ...
        // This function takes a string as parameter (one line of an PwDB data file) and outputs the transformed string, which must absolutely not contain a \n character (or data will be lost)
        ...
        // PwDB checks that never one is declared without the other, and checks that they are reverse of one another by testing on random strings of various lengths.
        // In addition, if too much data is detected as corrupt,
        // PwDB will refuse to start as it could mean you're not using the deserialization hook corresponding to the serialization hook used before (see below)
        beforeDeserialization?(line: string): string;
        // (optional): between 0 and 1, defaults to 10%. PwDB will refuse to start if more than this percentage of the datafile is corrupt.
